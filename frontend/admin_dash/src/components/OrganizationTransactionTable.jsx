...
const OrganizationTransactionTable = forwardRef(({ onLoaded }, ref) => {
  ...
  const handleSave = async () => {
    try {
      ...
      if (payload.length > 0) {
        setSaving(true);
        await API.post('/org_transactions/save', payload);
        setEdited({});
        setJustSaved(true);
  ...
  const renderCell = (rowIdx, col) => {
    ...
    return (
      <input
        ...
        style={{ borderBottom: '1px dashed #6c757d', borderRadius: 0 }}
        value={val}
  ...
  return (
    <div>
      ...
      <div className="d-flex justify-content-between align-items-center mt-2">
        ...
        <button className="btn btn-primary" onClick={handleSaveAndNext} disabled={saving || (currentPage === totalPages && Object.keys(edited).length === 0)} style={{ backgroundColor: '#6c5ce7', borderColor: '#6c5ce7' }}>
          {saving ? 'Saving...' : 'Next'}
        </button>
      </div>
    </div>
